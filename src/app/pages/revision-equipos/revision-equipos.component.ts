...
export class RevisionEquiposComponent implements OnInit {
  ...
  displayedColumns: string[] = ['cliente','equipo','problema','acciones'];
  ...
  showDetalles(datos){
    ...
    const dialogRef = this.dialog.open(ModalDetallesRevisionComponent, dialogConfig);
  ...
  showRevision(datos){
    ...
    const dialogRef = this.dialog.open(ModalRevisionComponent, dialogConfig);
