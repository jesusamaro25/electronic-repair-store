...
import { ModalPreguntaCalificacionComponent } from './modal-pregunta-calificacion/modal-pregunta-calificacion.component';
...
let PreguntaCalificacion: any[] = [];

@Component({
  selector: 'app-pregunta-calificacion',
  templateUrl: './pregunta-calificacion.component.html',
  styleUrls: ['./pregunta-calificacion.component.css']
})
export class PreguntaCalificacionComponent implements OnInit {
 ...
 // paginacion
  ...
  displayedColumns: string[] = ['pregunta','actualizar'];
  ...
  CargarTabla(){
      ...
      for (let i = 0; i < this.resultado.data.length; i++) {
        ...
        PreguntaCalificacion.push(this.datos);
      }
      this.dataSource = new MatTableDataSource(PreguntaCalificacion);
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
    }, (err) => {
      console.log(err);
...
showRegistrar() // abre una ventana modal para el registro
  {
    ...
    const dialogRef = this.dialog.open(ModalPreguntaCalificacionComponent, dialogConfig);
